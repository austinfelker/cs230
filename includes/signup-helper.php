        ...
        if (!mysqli_stmt_prepare($stmt, $sql)){
            ...
            if($check > 0){
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)){
                    ...
                    $hashed = password_hash($passw, PASSWORD_BCRYPT); #BCRYPT is the most secure from the default PHP options
                    mysqli_stmt_bind_param($stmt, "sssss",$lname, $fname, $email,$username,$hashed);
                    mysqli_stmt_execute($stmt);
                    mysqli_stmt_store_result($stmt);

                    $sqlImg = "INSERT_INTO profiles (uname, fname) VALUES ('$username', '$fname')";
